...
public class WebService {
	...
	{
		List<Urun> prods = entityManager.createQuery("select u from Urun u",Urun.class).getResultList();
		return prods;
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public List<Urun> getProductsByCategoryId(@PathParam("cid") int catID)
	{
		TypedQuery<Urun>  query =  entityManager.createQuery("select h from Urun h  where h.kategori.id=?1",Urun.class);
	...
	public String addProduct(Urun urun)
	{
		try 
		{
			...
			Kategori kategori = entityManager.find(Kategori.class, urun.getKategori().getId());
			...
			if (kategori != null) {
				...
				Kategori newKategori = new Kategori(urun.getKategori().getKategori());
				entityManager.persist(newKategori);
				urun.setKategori(newKategori);
				entityManager.persist(urun);
			...
			return "product couldn't be saved";
		}
	}
	...
	{
		List<Kategori> categories = entityManager.createQuery("select k from Kategori k",Kategori.class).getResultList();
		return categories;
	}
